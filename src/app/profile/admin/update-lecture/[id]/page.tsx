...
import { getModules, getSingleCourse } from "@/actions/get.action";
import { useUpdatedLecture } from "@/hooks/post.hook";
...
const UpdateLecture = ({ lectureId }: any) => {
  ...
  const { mutate: handleUpdateLecture } = useUpdatedLecture();
  ...
  useEffect(() => {
    const fetchLecture = async () => {
      const { data } = await getSingleCourse(lectureId);
      setLecture(data);
    };
  ...
  // Reset the form only after the lecture data has been set
  useEffect(() => {
    if (lecture) {
      reset({
    ...
    try {
      await handleUpdateLecture(formData, {
        onSuccess: () => {
          ...
          router.push("/lectures"); // Redirect to lectures list after update
        },
      });
    } catch (error) {
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-md">
      <h1 className="text-2xl font-bold text-gray-800 mb-6">Update Lecture</h1>
