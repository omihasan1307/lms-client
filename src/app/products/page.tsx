...
const fetchData = async (
  ...
  try {
    setIsLoading(true);
    const data = { search, category, sort, page };
...
const ProductList = () => {
  ...
  const search = useMemo(
    () => searchParams.get("search")?.replace(/_/g, " ") || "",
    [searchParams]
  );
  ...
  const debouncedSearch = useCallback(
    debounce((value: string) => {
      router.push(
        `/products?search=${value.replace(
          /\s+/g,
          "_"
        )}&category=${category}&sort=${sort}&page=1`
      );
    }, 500),
    [category, sort]
  );
  ...
  useEffect(() => {
    fetchData({ search, category, sort, page }, setIsLoading).then(
      (data: any) => {
  ...
  const paginationOptions = useMemo(
    () => ({ totalPages, totalItems, page, setPage }),
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                onChange={(e) =>
                  router.push(
                    `/products?search=${search}&category=${e.target.value}&sort=${sort}&page=1`
                  )
                }
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                onChange={(e) =>
                  router.push(
                    `/products?search=${search}&category=${category}&sort=${e.target.value}&page=1`
                  )
                }
                className="p-2 border border-gray-300 rounded"
              >
...
// Wrapping in Suspense to Handle `useSearchParams`
export default function WrappedProductList() {
  return (
